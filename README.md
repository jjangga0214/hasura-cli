...
Currently there are 3 npm tags (npm tags are different from versions), `latest`, `beta` and `alpha`. `latest` tag refers to Hasura's latest stable version(e.g. v1.3.3 as of writing), while `beta` and `alpha`, respectively beta version(e.g. v1.3.4-beta.2 as of writing) and alpha version(e.g. v1.4.0-alpha.1 as of writing).
